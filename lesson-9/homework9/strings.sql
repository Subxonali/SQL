...
SELECT
	...
	a.Salary,
	a.HireDate,
	isnull(b.Name,'CEO') as 'ManagerName'
...
SELECT*FROM Students s
 ...
 INNER JOIN Courses c on e.courseID=c.courseID where c.CourseName='Math 101'
...
SELECT*FROM Customers c
 inner join Orders o on c.CustomerID=o.CustomerID where o.Quantity>3
...
SELECT*FROM Employees e
left outer join  Departments d on e.DepartmentID=d.DepartmentID where d.DepartmentName='Human Resources'
...
SELECT e.DepartmentID, COUNT(e.DepartmentID) as Ishchilar_soni
FROM Employees e
inner join Departments d on e.DepartmentID=d.DepartmentID
...
LEFT OUTER JOIN Sales s on p.ProductID=s.ProductID where s.SaleAmount is null
...
SELECT a.EmployeeID AS Employee1_ID, a.Name AS Employee1_Name, 
       b.EmployeeID AS Employee2_ID, b.Name AS Employee2_Name, 
       a.ManagerID
FROM Employees a
...
INNER JOIN  Departments d on e.DepartmentID=d.DepartmentID and d.DepartmentName='Sales' and e.Salary>5000
...
INNER JOIN Departments d on e.DepartmentID=d.DepartmentID and d.DepartmentName='IT'
...
FULL OUTER JOIN Payments p on o.OrderID=p.OrderID where p.PaymentMethod is not null
...
LEFT OUTER JOIN Orders o on p.ProductID=o.ProductID where o.OrderID is null
...
WHERE p.ProductID IS NOT NULL AND c.CategoryID IS NULL;
